...
fieldNamesMotor = fieldnames(simlog_HEV_powersplit_adapted.Electrical.Motor);
if(~isempty(find(strcmp(fieldNamesMotor,'Detailed'))))

    % Reuse figure if it exists, else create new figure
    if ~exist('h2_HEV_powersplit_adapted', 'var') || ...
            ~isgraphics(h2_HEV_powersplit_adapted, 'figure')
        h2_HEV_powersplit_adapted = figure('Name', 'HEV_powersplit_adapted');
    end
    figure(h2_HEV_powersplit_adapted)
    clf(h2_HEV_powersplit_adapted)

    % Get simulation results

    simlog_t = simlog_HEV_powersplit_adapted.Vehicle.Vehicle_Body.v.series.time;
    simlog_iMot = simlog_HEV_powersplit_adapted.Electrical.Motor.Detailed.Meas_I.Current_Sensor.I.series.values('A');
    simlog_trq_ref = logsout_HEV_powersplit_adapted.get('torque_ref');
    simlog_trq_meas = logsout_HEV_powersplit_adapted.get('torque_meas');
    simlog_trq_estim = logsout_HEV_powersplit_adapted.get('torque_estim');
    simlog_trq_lim = logsout_HEV_powersplit_adapted.get('torque_lim');
